...
def readsvn(data, urli, match, proxy_dict):
    ...
    for a in data.text.splitlines():
        ...
        if a == "dir":
            if old_line != "":
                folder_path = os.path.join("Result/SvnRez", urli.replace("http://", "").replace("https://", "").replace("/", os.path.sep), old_line)
...
def readwc(data, urli, match, proxy_dict):
    folder = os.path.join("Result/SvnRez", urli.replace("http://", "").replace("https://", "").replace("/", os.path.sep))
    pattern = re.compile(match, re.IGNORECASE)
    global author_list
    global excludes
...
def save_url_wc(url, filename, svn_path, proxy_dict):
    ...
    if filename != "":
        if svn_path is None:
            folder_path = os.path.join("Result/SvnRez", url.replace("http://", "").replace("https://", "").replace("/", os.path.sep, filename.replace("/", os.path.sep)))
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)
        else:
            folder = os.path.join("Result/SvnRez", url.replace("http://", "").replace("https://", "").replace("/", os.path.sep), os.path.dirname(filename).replace("/", os.path.sep))
...
def save_url_svn(url, filename, proxy_dict):
    global author_list
    folder = os.path.join("Result/SvnRez", url.replace("http://", "").replace("https://", "").replace("/", os.path.sep))
    if not folder.endswith(os.path.sep):
        folder = folder + os.path.sep
    try:
...
This program actually automates the directory navigation and text extraction process"""
    ...
    if [200, 403, 500].count(r.status_code) > 0:
        print("URL is active")
        if no_extract:
            folder_path = os.path.join("Result/SvnRez", url.replace("http://", "").replace("https://", "").replace("/", os.path.sep))
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)
        if not x.entries:
